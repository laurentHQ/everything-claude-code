...
4. **Declarative-only safety in MVP.** `allow_mcp`, `allowed_mcp_servers`, `block_global_install`, `hook_profile:validation` are stored + validated by T1/T5 but **not runtime-enforced** until T6. Tests assert this gap explicitly (see `tests/integration/profile-conflict.test.js`). `docs/MVP-LIMITATIONS.md` records the gap. The pre-existing `ECC_DISABLED_MCPS` env-filter in `scripts/lib/install/apply.js` (parsed in `applyInstallPlan`, applied inside the `merge-json` and `copy-file` MCP dispatch handlers) continues to apply unchanged.
...
| 4 | 2561/2561 ✓ (+24) | `migrateInstallState` exported and called by `readInstallState`; `createInstallState` accepts `settings` + `backups` options; `createStatePreview` plumbs `profileSettings` from `createManifestInstallPlan`; `audit-log.js` exports `appendAuditEvent` / `maybeAppendAuditEvent` and is called from both `apply.js` and `install-lifecycle.js` uninstall path (try/catch wrapped); `copy-path` aliases `copy-file` in lifecycle via `isFileCopyKind` helper | v1 state file on disk migrates to v2 on `readInstallState` (verified manually); `audit.jsonl` written only when `settings.require_audit_log: true`, no write when false; audit entry contains timestamp + action + profileId; install-state schema enforces enum `kind` and optional `settings`/`backups` | Fixture v1 → v2 bumps in 3 existing tests (`install-state.test.js`, `install-apply.test.js`, `path-safety.test.js`) were applied directly rather than via `migrateInstallState` — pragmatic but a tech-debt item: a future test could lock the migration boundary by keeping a v1 fixture verbatim. | `4b4f5c4c` |
| 5 | 2588/2588 ✓ (+27) | snapshot matrix iterates over `(profile, target)`; integration tests use `createManifestInstallPlan` + `applyInstallPlan` directly; round-trip + no-writes-outside-tmpdir test exercise the lifecycle T5 enabled; `generate-snapshots.js` is a maintenance utility, not a test | 14 snapshots normalized with `<HOME>`/`<PROJECT>`/`<REPO>` (verified no leakage of host paths); `security` snapshot's `safety` block reflects declarative settings (`mcpAllowed:false`, `globalInstallAllowed:false`); `profile-conflict.test.js` pins MVP gap — `mcp-not-allowed`/`global-install-blocked` conflicts NOT emitted yet (will land in T6); round-trip restores fake-home baseline for `minimal × {claude, codex}` | Snapshots use `\n` line endings + 2-space indent; consistent with project style. No production code touched. | `5a043edd` |
